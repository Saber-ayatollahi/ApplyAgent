...
## Target role
Director, IRRBB & FTP Model Risk Management
...
```json
{
  ...
  "tier": 1,
  "title": "Director, IRRBB & FTP Model Risk Management",
  "level": "Director",
  "url": "https://ca.linkedin.com/jobs/view/director-irrbb-ftp-model-risk-management-at-rbc-4408795177",
  "portal_url": "https://ca.linkedin.com/jobs/view/",
  "date_found": "2026-05-03",
  "date_jd_verified": "2026-05-03",
  "date_applied": null,
  "date_last_followup": null,
  "source": "scraper_v2+v3_tier1",
  "status": "Tailoring",
  "fit_score": "High",
  "fit_score_numeric": 5,
  "osfi_hook": "B-12 IRRBB revision (Q1 2026 consultations)",
  "urgency": "High",
  "expected_comp_band_cad": "$210K-$270K CAD base + 40-55% bonus + LTIP",
  "fit_notes": "CRITICAL find \u2014 near-verbatim match for Saber's profile. IRRBB AND FTP AND Model Risk at Director level. B-12 tailwind directly applies. Apply this week.",
  "keywords": [
    "IRRBB",
    "FTP",
    "model risk",
  ...
  "rejection_date": null,
  "next_action": "Apply this week \u2014 draft tailored resume + cover letter via jd_tailor; find warm intro before submit.",
  "notes": "Surfaced by scraper v2/v3 Tier-1 ranking 2026-05-03."
}
```

## Deterministic JD analysis (pre-computed, do not re-derive)
- coverage: **80%** (8 skill hits, 2 gap hits)
- suggested variants (by bullet+skill support): ALM (5b/4s), VEN (3b/3s), VAL (2b/2s)
- skills the JD calls for that Saber has evidence for:
  - `sk_alm` (expert, 7y) **[PRIMARY]** — matched on "asset liability management"
  - `sk_irrbb` (expert, 6y) **[PRIMARY]** — matched on "interest rate risk in the banking book (IRRBB)"
  - `sk_model_review_challenge` (expert, 4y) **[PRIMARY]** — matched on "model validation"
  - `sk_client_onboarding` (expert, 1y) **[PRIMARY]** — matched on "implementation"
  - `sk_python` (expert, 7y) — matched on "Python"
  - `sk_excel` (expert, 7y) — matched on "Excel"
  - `sk_sql` (proficient, 4y) — matched on "SQL"
  - `sk_investment_risk_oversight` (proficient, 3y) — matched on "Investment Risk"
- **GAPS flagged** — JD mentions these, Saber is NOT hands-on:
  - "QRM" (gap: QRM (hands-on))
  - "express" (gap: TypeScript / Node.js / React (production delivery))
- lane-probe negatives (not in JD, informational): ALM, stress testing, aladdin, bloomberg, msci, derivatives, fixed income

## Pre-ranked bullet shortlist (select from here; highest-ranked first)

### role_moodys
- `b_moodys_signoff_signoff_curve` (score=5.5) [ALM][VAL] [matches: sk_alm, sk_model_review_challenge]
  > Held sign-off authority on valuation, sensitivity, and ALM outputs delivered to institutional client portfolios ranging $5–25bn per engagement — independently reviewed curve construction, spread calibration, and cross-asset interactions prior to production release.
- `b_moodys_spreadsheet_to_python` (score=4.0) [VEN] [matches: sk_model_review_challenge, sk_python]
  > Re-engineered manual spreadsheet workflows into scalable Python analytics pipelines with embedded logging, validation, and auditability controls.
- `b_moodys_irrbb_shocks` (score=3.0) [ALM] [matches: sk_irrbb]
  > Oversaw interest-rate risk and duration analysis under parallel and non-parallel rate shocks, aligned with IRRBB standards analogous to OSFI B-12 and Basel Committee frameworks.
- `b_moodys_led_cashflow_engine` (score=3.0) [ALM] [matches: sk_alm]
  > Led design and implementation of an enterprise multi-asset cash flow projection engine supporting base, stress, and reverse-stress scenarios — delivering forward-looking liquidity visibility across ALM horizons.
- `b_moodys_onboarding_pfaroe` (score=3.0) [VEN] [matches: sk_client_onboarding]
  > Led onboarding and implementation for U.S. and Canadian pension funds, asset managers, and consulting firms migrating to an enterprise analytics platform — served as technical liaison between client investment teams and product/development functions.
- `b_moodys_calypso_migration` (score=3.0) [VEN] [matches: sk_client_onboarding]
  > Successfully migrated all assigned client accounts from a legacy Calypso environment to a modern PFaroe PM platform, validating model outputs post-deployment.
- `b_moodys_model_governance_framework` (score=2.0) [VAL] [matches: sk_model_review_challenge]
  > Operated within formal model-governance framework — reviewed assumptions, validated economic defensibility, escalated outputs lacking mathematical support; built documentation standards consistent with SR 11-7 and OSFI E-23 expectations.

### role_ortec
- `b_ortec_alm_studies` (score=3.0) [ALM] [matches: sk_alm]
  > Delivered ALM studies for Canadian and international pension funds and institutional investors — analyzed funding ratios, duration mismatch, and long-term balance-sheet risk exposures.
- `b_ortec_saa_taa` (score=2.0) [ALM][CON] [matches: sk_investment_risk_oversight]
  > Advised clients on Strategic Asset Allocation (SAA) and Tactical Asset Allocation (TAA) trade-offs via stress testing and sensitivity analysis.

## Job description
```
Job Description
What is the opportunity?
The Group Risk Management (GRM) Balance Sheet and Liquidity Risk (BSLR) team performs the second line of defense role for all balance sheet and liquidity risks at the enterprise level.
As Director, IRRBB & FTP Model Risk Management, you will lead the vision and strategy for independent oversight of methodologies, parameters, assumptions, and models used for measuring interest rate risk in the banking book (IRRBB) and fund transfer pricing.
...
What will you do?
Oversee validation activities for IRRBB and FTP models and component customer behavioral models such as mortgage prepayment, non-maturity deposit, and commitment models across the enterprise
Ensure the team delivers timely and effective assessments of model methodology, assumptions, limitations, data quality, implementation, and performance monitoring frameworks.
Lead the design and methodology of validation testing and robustness assessments, establishing testing protocols and standards to evaluate IRRBB and FTP model effectiveness across the enterprise.
Ensure validation reports are comprehensive and of high-quality to inform senior management and governance forums on model performance, risk, and findings.
Engage with model owners, developers, users, and stakeholders to proactively identify, assess, monitor, and manage model-related concerns or findings at an enterprise level.
Collaborate with model governance, senior risk managers, audit teams, and business partners to ensure validation strategies and recommendations are well-informed by business context and aligned with regulatory requirements.
Lead efforts to ensure that RBC's Balance Sheet and Liquidity Risk frameworks and policies meet local regulatory requirements such as OSFI B12 and align with business strategies and industry best practices.
Manage team work plans, resource allocation, and project prioritization to ensure completion in accordance with committed timelines and regulatory deadlines, including regulatory reviews, inspections, audits, and other governance deliverables.
Implement corporate and divisional strategic plans and budgets for the BSLR IRRBB & FTP validation function, ensuring alignment with enterprise priorities and regulatory requirements.
Manage activities and performance of senior managers and managers reporting to the Director, providing clear direction, accountability, and support for execution of validation programs.
Recruit, develop, and mentor team members in model validation activities, fostering their technical expertise and leadership capabilities while embodying RBC values of collaboration, respect, and excellence.
Keep abreast of industry trends and regulatory developments on IRRBB and interest rate risk management, translating emerging insights into organizational strategy and practice improvements.
What do you need to succeed?
Advanced university degree in a financial or quantitative discipline (such as financial engineering, mathematics, statistics, economics, finance, engineering, computer science, physics, or any equivalent discipline).
5+ years of work experience in financial industry, with at least 3 years in model validation, financial modeling, treasury asset liability management, or interest rate risk management roles with demonstrated progressive responsibility.
Strong computational, data analytical, and critical thinking skills. Advanced proficiency in Python, Excel, and SQL.
Strong knowledge of general banking products and IRRBB regulatory requirements such as OSFI B12.
Comprehensive understanding of RBC's businesses and balance sheet structure, with particular knowledge of interest rate risk and customer behavioral dynamics.
Proven experience leading and developing technical teams in a risk or model validation environment.
Excellent collaborative mindset and superb interpersonal skills, with strong verbal and written communication abilities suitable for executive-level interactions.
Track record of establishing frameworks, policies, or standards that have improved organizational practices or governance.
Strong organizational and planning skills with demonstrated ability to prioritize across competing regulatory and business initiatives.
Relevant professional designations (CFA, FRM, or similar).
Experience in regulatory examination or compliance roles related to IRRBB.
Nice-to-Have:
...
Job Skills
Credit Analysis, Financial Regulation, Internal Auditing, Investment Risk Management, Liquidity Risk, Market Risk, Model Risk, Risk Management
...
, 
 ...
 Directeur ou directrice, Gestion des risques – Indépendance
 ...
 Director, International Banking - Digital Application Fraud Risk
 ...
 Associate Director, Capital Markets & Payments Risk Advisory, AML and Sanctions - Toronto
 ...
 Director Risk Services Eastern Region
 ...
 Director, Project and Infrastructure Finance, Corporate Client Group
 ...
 Director, Enterprise & Financial Risk Management
 ...
 Associate Director – Corporate and Construction Insurance
 ...
 Director, Finance Operations, Franchise
 ...
 Head, North American Provisioning & Risk Analytics
 ...
 Premier directeur, Risque au titre de la protection des renseignements personnels, RH – Amérique du Nord
 ...
 Senior Vice President Risk Management jobs
 ...
 Chief Underwriting Officer jobs
 ...
 Risk Management Coordinator jobs
 ...
 Director Credit Risk jobs

 52 open jobs

 Director of Compliance jobs
 ...
 Senior Manager Risk Management jobs
